...
try
    ...
    load(['theta_locations/ring_locations_L_',num2str(L),'.mat']);
    return
catch err
    disp('computing optimal ring placement');
%     %optimally place rings 
    
    NUM_RINGS = (L+1)/2; %number of rings to take physical samples over
    TT = nsht_indexed_theta(L);
    ...
    save(['theta_locations\ring_locations_L_' num2str(L) '.mat'], 'TT_updated', 'min_cond_vec'); 
