%obtained from the 2012 HARDI reconstruction challenge. Creates instance of
%multi-tensor class which is a intra-voxel diffusion model.
...
methods
	...
	% Constructor
	% sets up default values for properties
	...
	% ---------------------------------------------
	function [signal, signal_noisy] = acquireWithScheme( obj, grad_list, sigma )
		if isstr(grad_list)
		...
		nDIR   = size( XYZ, 1 );
		signal = zeros( nDIR, 1 );
        signal_noisy = zeros( nDIR, 1 );
		for d = 1:nDIR
			signal(d) = obj.E( XYZ(d,4) * XYZ(d,1:3)' );
			signal_noisy(d) = obj.addNoise( signal(d), sigma );
		end
	end
